...
import { getAllRoles, getRoleById, createRole, updateRole, deleteRole, logActivity } from '../database.js';
import { authenticate, requirePermission, getAllPermissions } from '../auth.js';
const router = Router();
// 取得所有權限列表
...
router.put('/:id', authenticate, requirePermission('roles.manage'), (req, res) => {
    try {
        ...
        const updatedRole = getRoleById(id);
        res.json({
            success: true,
