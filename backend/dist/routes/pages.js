...
import { getAllPages, getPageById, getPageBySlug, createPage, updatePage, deletePage, logActivity } from '../database.js';
...
// 建立頁面
router.post('/', authenticate, requirePermission('pages.create'), (req, res) => {
    try {
...
// 刪除頁面
router.delete('/:id', authenticate, requirePermission('pages.delete'), (req, res) => {
    try {
        const id = req.params.id;
        const page = getPageById(id);
        if (!page) {
